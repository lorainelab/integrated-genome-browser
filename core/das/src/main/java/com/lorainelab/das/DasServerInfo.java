...
public final class DasServerInfo {
    ...
    private DataProvider primaryServer = null;
     ...
     * @param url
     */
    public DasServerInfo(URL url) {
        serverURL = url;
    }

    public Map<String, DasSource> getDataSources(URL primaryURL, DataProvider primaryServer) {
    ...
    private void setPrimaryURL(URL primaryURL) {
        if (primaryURL != null) {
            try {
                this.primaryURL = new URL(primaryURL.toExternalForm());
    ...
    public InputStream getInputStream(Map<String, List<String>> headers, String log_string) throws IOException {
        ...
        if (istr == null && isLoadingFromPrimary()) {
            ...
            logger.warn("Primary Server :{} is not responding. So disabling it for this session.", primaryServer.getName());
    ...
    private boolean isLoadingFromPrimary() {
        return (primaryURL != null && primaryServer != null && !primaryServer.getStatus().equals(NotResponding));
    }
