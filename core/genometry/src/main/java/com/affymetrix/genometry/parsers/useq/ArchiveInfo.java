...
/**
 * This contains information related to all of the data slices in a USeq archive. It is always called archiveReadMe.xxx
 * and is the first ZipEntry in the archive.
 * The format of the archiveReadMe.txt version is simply comment lines beginning with '#' that are not parsed and
 * key=values delimited by a return, thus one per line.
 * The first '=' sign in each key=value is used to split the tokens. Key's must not contain '=' signs or white space.
 * White space before and after the '=' are permitted.
 * Where possible do use the reserve key names and add new ones as needed. At some point a archiveReadMe.xml version
 * with a DTD should be created, volunteers?
 *
...
public class ArchiveInfo {
    ...
    /**
     * Currently just two types exist graph and region. This is used as to indicate how the data should be displayed
     * (e.g. as a graph, or as
