...
public class USeq2Text {
    ...
    /**
     * Prints a UseqArchive binary file to a text file in either minimal native format (see comment at beginning of file
     * for a description of the columns) or in bed6 or bed12 format.
     */
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (mat.matches()) {
                ...
                try {
                    switch (test) {
                        case 'f':
                            useqArchives = USeqUtilities.extractFiles(new File(args[++i]), USeqUtilities.USEQ_EXTENSION_NO_PERIOD);
                            break;
                        case 'b':
                            printBedFormat = true;
                            break;
                        case 'c':
                            convertScoresToBedFormat = true;
                            break;
                        case 'w':
                            printWigFormat = true;
                            printBedFormat = false;
                            break;
                        case 'h':
                            printDocs();
                            System.exit(0);
