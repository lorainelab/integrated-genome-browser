...
import com.affymetrix.genometryImpl.symmetry.impl.GenbankSym;
...
public final class Genbank extends SymLoader {
    ...
    /**
     * There are absolutely no mandatory qualifiers for an mRNA feature so that
     * makes finding an appropriate value to use as an ID/Name a bit
     * problematic. Do the best we can and if that fails then make something up.
     * This method however, returns null if there are no ID-able qualifiers
     * found. Optional qualifiers /allele="text" (and in gene)
     * /citation=[number] (and in gene) /db_xref="<database>:<identifier>" (and
     * in gene) /evidence=<evidence_value> (and in gene) /function="text" (and
     * in gene) /gene="text" (and in gene) /label=feature_label (and in gene)
     * /locus_tag="text" (single token) (and in gene) /map="text" (and in gene)
     * /note="text" (and in gene) /operon="text" (and in gene) /product="text"
    ...
    /**
     * The column of the output where we should start writing the location. e.g.
...
//		strategyList.add(LoadStrategy.CHROMOSOME);
    ...
    /**
     * Sets up all the current values, the line itself, the line type, the
     * offset, and the content
     */
...
final class GenbankFeature {
  ...
  // this is a hash into vectors to allow for cases where
    ...
    /**
     * location operators ::= complement, join, order locations ::=
     * basenumber^basenumber (insertion between these) basenumber..basenumber
     * (span) basenumber.basenumber (single base) basenumber (single base)
     * adding < and > now too entryname:basenumber..basenumber (indirect
     * references)
     *
     *
     *
