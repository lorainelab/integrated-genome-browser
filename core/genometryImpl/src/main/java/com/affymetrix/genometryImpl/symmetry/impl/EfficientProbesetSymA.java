...
public final class EfficientProbesetSymA implements SeqSpan, SymWithProps, IntId {
     ...
     * routine if this is not the case. This means that the ordering of the
     * elements in the array you pass in may be altered as a side-effect.)
    ...
    public MutableSeqSpan getChildSpan(int child_index, BioSeq aseq, MutableSeqSpan result_span) {
        ...
        if (forward) {
            result_span.set(child_mins[child_index], child_mins[child_index] + getProbeLength(), aseq);
        } else {
            result_span.set(child_mins[child_index] + getProbeLength(), child_mins[child_index], aseq);
        }
        return result_span;
    }
    ...
    public int getIntID() {
        return nid;
    } // implementing IntId interface

    public int getProbeLength() {
        return info.getProbeLength();
    }

    public String getIDPrefix() {
        return info.getIDPrefix();
    }
    ...
    /**
     * WARNING: The implementation of the Propertied (SymWithProps) interface in
     * this class is incomplete and is very likely to change or be removed in
     * future implementations. Returns a new Map instance with only two values:
     * "method" maps to "HuEx-1_0-st-Probes"; and "id" maps to the value of
     * getID().
     */
