 ...
 * {@link IndexedSym}. Assumes that ScoredContainerSym has only one SeqSpan
 */
public class ScoredContainerSym extends RootSeqSymmetry {
     ...
     * already been set.
     *
     * @param scores a float array with the same length as the number of
     * children.
     */
    ...
    /**
     * Creates a GraphIntervalSym. Assumes all child syms have already been
     * added, and span has already been set. Resultant graph sym has x, width,
     * and y data points for each child sym.
     *
    ...
    /**
     * Make a GraphSym, but only with scores for scored intervals in the
    ...
    private static ITrackStyleExtended newComboStyle(String name) {
        ITrackStyleExtended style = DefaultStateProvider.getGlobalStateProvider().getAnnotStyle(name);
