package org.lorainelab.igb.genoviz.extensions.glyph;
...
//	ResidueColorHelper helper = null;
     ...
     * the glyph borders. This method assumes the tiers are horizontal. The left
     * and right border are taken from the view's pixel box. The top and bottom
     * border are from the coord box.
     */
    @Override
...
//	}
    /**
     * Designed to work in combination with pickTraversal(). If called outside
     * of pickTraversal(), may get the wrong answer since won't currently take
     * account of nested transforms, etc.
     */
    @Override
    ...
    /**
     * This number is calculated in {@link #checkVisibleBoundsY} directly from y
     * coordinates, and cannot be modified (except by calling
     * checkVisibleBoundsY() again).
     *
     * @return maximum y coordinate (in graph coordinates) of all points in
     * graph.
     */
    ...
    /**
     * getVisibleMaxY() returns max ycoord (in graph coords) that is visible
     * (rendered). This number can be modified via calls to setVisibleMaxY, and
    ...
    /**
     * If want to override default setting of y min and max (based on
     * setPointCoords()), then must call setMaxY() and setMinY() _after_ call to
     * setPointCoords() -- any subsequent call to setPointCoords will again
     * reset y min and max.
     */
    ...
    /**
     * Same as GraphGlyph.getInternalLinearTransform(), except also calculates a
     * bottom y offset for showing thresholded regions, if
     * showThresholdedRegions() == true.
     */
