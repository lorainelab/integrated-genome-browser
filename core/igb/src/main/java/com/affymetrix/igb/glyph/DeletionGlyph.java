...
public class DeletionGlyph extends SolidGlyph {
     ...
     * sym/annotation's children are outside the bounds of the current view, but
     * due to transformations the information about which edge is closest to is
     * not in the sym itself, but can be found by comparison with sym that view
     * is based on
     *
     * @param outside_children children syms that are outside edges of view
     * defined by coordseq
     * @param pglyph parent glyph
     * @param annotseq original annotated seq
     * @param coordseq seq for rendering view (could be same as annotated seq or
     * different)
    ...
    public static void handleEdgeRendering(java.util.List<SeqSymmetry> outside_children, GlyphI pglyph, BioSeq annotseq, BioSeq coordseq, double deletion_y, double deletion_height) {
         ...
         * was that previous code was assuming that if any children were out of
         * the slice view, then either: first child is out on left (5') side of
         * view last child is out on right (3') side of view or both But
         * ordering of children within a parent cannot be assumed So instead,
         * now checking bounds of child's original coords relative to
         * composition coords of entire slice view
         */
