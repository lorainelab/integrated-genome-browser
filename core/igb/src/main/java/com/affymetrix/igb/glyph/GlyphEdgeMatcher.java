...
public class GlyphEdgeMatcher {
    ...
    static GlyphI getTier(GlyphI target) {
        GlyphI p = target;
        while (p.getParent() != null && !(p instanceof TierGlyph)) {
            p = p.getParent();
        }
        return p;
    }
    ...
    /**
     * Recursive glyph edge-matching. Recursively descends, only matches leaf
     * nodes (getChildren() == null || getChildren().size()<1)
     *
     * Glyphs present in both query_glyphs and target_glyphs will match each
     * other (no filtering against matching self).
     *
...
//		}
    ...
    void addEdgeMatch(NeoMap map, GlyphI query, GlyphI target, List<GlyphI> match_glyphs) {
        Rectangle2D.Double qbox = query.getCoordBox();
        Rectangle2D.Double tbox = target.getCoordBox();
