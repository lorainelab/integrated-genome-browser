...
import com.affymetrix.igb.view.load.GeneralLoadUtilsService;
 ...
 */
@Component(name = OrfAnalyzer.COMPONENT_NAME, provide = OrfAnalyzer.class)
public final class OrfAnalyzer extends JComponent
        implements ChangeListener, ActionListener {

    public static final String COMPONENT_NAME = "OrfAnalyzer";
    ...
    private AltSpliceView altSpliceView;
    private GeneralLoadUtilsService generalLoadUtils;

    @Reference
    public void setAltSpliceView(AltSpliceView altSpliceView) {
        this.altSpliceView = altSpliceView;
    }

    @Reference(target = "isAltSpliceSeqMapView=true")
    public void setView(SeqMapViewI view) {
        this.smv = view;
    }
    ...
    public void actionPerformed(ActionEvent evt) {
        Object src = evt.getSource();
        if (src == showCB) {
            JRPTextField tf = altSpliceView.getBufferSizeTF();
            ...
            if (show_orfs) {
                ...
                altSpliceView.setSliceBuffer(0);
            } else {
                altSpliceView.setSliceBuffer(previousBefferSize);
    ...
    void redoOrfs() {
        ...
        if (!(seq.isAvailable(aspan))) {
            //Load Residues
            if (!generalLoadUtils.loadResidues(aspan, true)) {
                show_orfs = false;
                showCB.setSelected(false);
                return;
            }
