...
public class ParentUnionOperator extends AbstractAnnotationOperator implements Operator {
    ...
    public ParentUnionOperator(FileTypeCategory fileTypeCategory) {
        super(fileTypeCategory);
    }

    @Override
    public String getName() {
        return category.toString().toLowerCase() + "_parent_union";
    }
    ...
    public SeqSymmetry operate(BioSeq aseq, List<SeqSymmetry> symList) {
        ...
        GraphSym landscape = SeqSymSummarizer.getSymmetrySummary(allSyms, aseq, true, "", 2);
        // now just flatten it
        if (landscape != null) {
