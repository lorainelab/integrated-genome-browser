 ...
 * Use {@link com.affymetrix.genoviz.bioviews.GlyphI#setForegroundColor(java.awt.Color)} to set the main glyph color.
...
public class CornerGlyph extends DirectedGlyph {
  ...
  public void draw(ViewI view)
  {
    ...
    if (pixelbox.width <MIN_PIXELS) {
      pixelbox.width = MIN_PIXELS;
    }
    if (pixelbox.height <MIN_PIXELS) {
      pixelbox.height = MIN_PIXELS;
    }
    ...
    case Horizontal:
      ...
      if (draw_patch) {
        if (patch_color !=  getForegroundColor()) {
        g.setColor(patch_color);
      }
        int one_half = Math.max(pixelbox.width/2, MIN_PIXELS);
        if (isForward()) {
        g.fillRect(x1 - one_half, pixelbox.y + pixelbox.height / 3, one_half, pixelbox.height / 3);
      }
        else {
        g.fillRect(x1, pixelbox.y + pixelbox.height / 3, one_half, pixelbox.height / 3);
      }
      }
    ...
    case Vertical:
      ...
      if (draw_patch) {
        ...
        if (isForward()) {
        g.fillRect(pixelbox.x + pixelbox.width / 3, y1, pixelbox.width / 3, one_half);
      }
        else {
        g.fillRect(pixelbox.x + pixelbox.width / 3, y1, pixelbox.width / 3, one_half);
      }
  ...
  /**
   * @deprecated use {@link com.affymetrix.genoviz.bioviews.Glyph#setForegroundColor(java.awt.Color)}.
   */
  @Deprecated
  @Override
  ...
  /**
   * @deprecated use {@link com.affymetrix.genoviz.bioviews.Glyph#getForegroundColor()}.
   */
  @Deprecated
  @Override
