...
public class PslLinkConstructor {
  List link_chunklists = new ArrayList();
  List link_labels = new ArrayList();
  Map label2chunks = new HashMap();

  public PslLinkConstructor()  {
  }
  
  public void addLink(String link_label, List chunklist) {
  ...
  public String getLinkLabel(int index) {
    return (String)link_labels.get(index);
  }

  public String getLink(int link_index, UcscPslSym sym) {
    List chunks = (List)link_chunklists.get(link_index);
    return this.getLink(chunks, sym);
  }

  public String getLink(String link_label, UcscPslSym sym) {
    List chunks = (List)label2chunks.get(link_label);
    return this.getLink(chunks, sym);
  }

  protected String getLink(List chunks, UcscPslSym sym) {
