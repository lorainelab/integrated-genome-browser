...
public class TierResizer extends MouseInputAdapter {
	...
	private boolean manualResizingAllowed = true; // for the moment. remove this later
	 ...
	 * @return a maximal (possibly empty) expansion of theList.sublist(theFirst,
	 * theLast+1) such that the first and last tiers can be resized and none of
	 * the others can.
	 */
	...
	public void mouseMoved(MouseEvent theEvent) {
		if (theEvent instanceof NeoMouseEvent) {
			...
			if (src instanceof AffyTieredMap) {
				AffyTieredMap m = (AffyTieredMap) src;
				if (atResizeTop(nevt)) {
	...
	public void mousePressed(MouseEvent evt) {
		if (evt instanceof NeoMouseEvent) {
			...
			AffyTieredMap labelmap = (AffyTieredMap) nevt.getSource();
...
//					resizeBorder(resizeRegion, nevt);
	...
	public void mouseReleased(MouseEvent evt) {
		...
		if (needRepacking) {
			...
			// The border width of 2 pixels looks suspicious both here and when moving the lower split pane.
			// - elb
