...
public class SeqResiduesLoader {
	...
	if (seq_group instanceof Das2SeqGroup)  {
	    ...
	    try {
		istr = LocalUrlCacher.getInputStream(bnib_uri, 
						     QuickLoadServerModel.getCacheResidues(), 
						     headers);
		...
		if ((istr != null) && 
		    (content_type != null) && 
		    (content_type.equals(NibbleResiduesParser.getMimeType())))  {  // check for bnib format
		...
		try { if (istr != null) istr.close(); } catch (Exception e) { e.printStackTrace(); }
	    }
