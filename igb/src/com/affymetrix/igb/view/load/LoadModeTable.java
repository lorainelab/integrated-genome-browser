...
public final class LoadModeTable {
	...
	static final Icon invisible_icon = IGBUtils.getIcon("invisible.png");
	static final Icon visible_icon = IGBUtils.getIcon("visible.png");
	...
	static void setComboBoxEditors(JTableX table, boolean enabled) {
		...
		table.setRowEditorModel(LoadModeDataTableModel.COL_BACKGROUND, color);
		table.setRowEditorModel(LoadModeDataTableModel.COL_FOREGROUND, color);
...
class JTableX extends JTable implements MouseListener {
	...
	public TableCellRenderer getCellRenderer(int row, int column) {
		...
		if (column == LoadModeDataTableModel.REFRESH_FEATURE_COLUMN) {
			if (ftm.subFeatures.contains(feature)) {
			...
			for (TierGlyph tier : currentTiers) {
				if (tier.getAnnotStyle().getUniqueName() != null) {
					if (tier.getAnnotStyle().getUniqueName().equalsIgnoreCase(
							feature.uniqueName))//need changed
					{
	...
	public String getToolTipText(MouseEvent e) {
		...
		switch (realColumnIndex) {
			...
			case LoadModeDataTableModel.FEATURE_NAME_COLUMN:
				tip = "File Name " + feature.gVersion.gServer.serverName + " (" + feature.gVersion.gServer.serverType + ")" + "\n " + feature.featureName;
				break;
			...
			case LoadModeDataTableModel.TRACK_NAME_COLUMN:
				tip = "Track Name";
				break;
			case LoadModeDataTableModel.COL_BACKGROUND:
				tip = "Background";
				break;
			case LoadModeDataTableModel.COL_FOREGROUND:
				tip = "Foreground";
				break;
			default:
				tip = "";
		...
		switch (realIndex) {
			...
			case LoadModeDataTableModel.DELETE_FEATURE_COLUMN:
				...
				if (featureSize > 0 && Application.confirmPanel(message, PreferenceUtils.getTopNode(),
					...
					for (int row = 0; row < featureSize; row++) {
						GeneralLoadView.getLoadView().removeFeature(ftm.getFeature(row), true);
					}
					ftm.subFeatures.clear();
					ftm.allFeatures.clear();
				}
				break;
			case LoadModeDataTableModel.HIDE_FEATURE_COLUMN:
