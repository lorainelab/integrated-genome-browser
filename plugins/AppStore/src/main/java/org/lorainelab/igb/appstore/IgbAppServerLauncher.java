 ...
 */
public final class SimpleAppMangerServer {

    private static final int NO_PORT = -1;
    private static final int PORTS_TO_TRY = 1;
    private static int server_port = NO_PORT;
    public static final int DEFAULT_APP_MANAGER_SERVER_PORT = 7086;
    private static final Logger logger = LoggerFactory.getLogger(SimpleAppMangerServer.class);
    ...
    public static void init(final IgbService igbService, WebAppManager webApp) {
        startServerSocket(igbService, DEFAULT_APP_MANAGER_SERVER_PORT,webApp);
        if (server_port != NO_PORT) {
            startServerSocket(igbService, server_port, webApp);
        }
    }

    public static void startServerSocket(final IgbService igbService, int startPort, WebAppManager webApp) {
        try {
            ...
            if (serverPort == NO_PORT) {
                ...
                Runnable r = () -> {
                    AppManagerHttpRequestHandler handler = new AppManagerHttpRequestHandler(igbService, serverPort,webApp);
                    try {
                        handler.start();
                    } catch (IOException ex) {
                        logger.error("Could not start App Manager  server, turning off IGB's URL-based control features.");
                    }
                };
