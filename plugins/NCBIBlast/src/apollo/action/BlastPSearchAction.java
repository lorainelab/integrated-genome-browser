 ...
 * @author hiralv
 */
public class BlastPSearchAction extends GenericAction {
	private static final long serialVersionUID = 1l;
	
	private final SeqMapViewI smv;
	
	public BlastPSearchAction(SeqMapViewI smv) {
		super(RemoteBlastNCBI.BlastType.blastp.toString().toUpperCase() + " nr protein database", null, null);
		this.smv = smv;
	}
			...
			final GenericActionDoneCallback doneback = new GenericActionDoneCallback() {
				public void actionDone(GenericAction action) {
					try {
						...
						String proteinAnnotation = AminoAcid.getAminoAcid(residues, 1, span.isForward(), "");
						StrandedFeatureSetI sf = new StrandedFeatureSet();
						Sequence seq = new Sequence(aseq.getID(), proteinAnnotation);
						
						RemoteBlastNCBI blast = new RemoteBlastNCBI(RemoteBlastNCBI.BlastType.blastp, new RemoteBlastNCBI.BlastOptions());
