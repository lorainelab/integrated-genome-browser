    ...
    private void addFormattedResidues() {
        ...
        while (it_working.hasNext()) {
            ...
            if (cv.getSi().getCdsStart() >= 0) {
                if (toggleReverseComplement) {
                    seqview.addOutlineAnnotation(start + revCdsStart - 3, start + revCdsStart - 1, colorSwitch?Color.green.darker().darker():Color.green);
                    seqview.setCdsStart(start + revCdsStart - 3);
                } else {
                    seqview.addOutlineAnnotation(start + cdsStart, start + cdsStart + 2, colorSwitch?Color.green.darker().darker():Color.green);
