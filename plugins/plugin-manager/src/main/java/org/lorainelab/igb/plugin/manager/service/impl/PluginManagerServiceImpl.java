...
public class PluginManagerServiceImpl implements PluginManagerService {
    ...
    public Response manageApp(JsonObject requestBody) {
        ...
        final PluginListItemMetadata plugin = appManager.getPluginItems().stream()
...
//                    appManagerstView().setItems(appManagerFxPanel.getListView().getItems());
            ...
            if (plugin.isInstalled()) {
                return createManageAppResponse(AppStatus.INSTALLED.toString(), plugin.getVersion(), plugin.getBundle().getSymbolicName(), Status.OK);
            }

        } catch (InterruptedException | ExecutionException ex) {
            java.util.logging.Logger.getLogger(PluginManagerServiceImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
        return createManageAppResponse(AppStatus.ERROR.toString(), plugin.getVersion(), plugin.getBundle().getSymbolicName(), Status.INTERNAL_ERROR);
    }
    ...
    private Response uninstallApp(PluginListItemMetadata plugin) {
        ...
        try {
            ...
            if (!plugin.isUpdateable()) {
                return createManageAppResponse(AppStatus.UNINSTALLED.toString(), plugin.getVersion(), plugin.getBundle().getSymbolicName(), Status.OK);
            }
    ...
    private Response updateApp(PluginListItemMetadata plugin) {
        try {
            ...
            logger.info("Updated App {} version {} from {}", plugin.getBundle().getSymbolicName(), plugin.getVersion(),
                    plugin.getRepository());
            if (!plugin.isUpdateable()) {
                return createManageAppResponse(AppStatus.UPDATED.toString(), plugin.getVersion(), plugin.getBundle().getSymbolicName(), Status.OK);
            }
    ...
    private Response getAppInfo(PluginListItemMetadata plugin) {
        ...
        String appStatus = plugin.isUpdateable() ? AppStatus.TO_UPDATE.toString()
                : (plugin.isInstalled() ? AppStatus.INSTALLED.toString()
                : AppStatus.UNINSTALLED.toString());
        return createManageAppResponse(appStatus, plugin.getVersion(), plugin.getBundle().getSymbolicName(), Status.OK);
