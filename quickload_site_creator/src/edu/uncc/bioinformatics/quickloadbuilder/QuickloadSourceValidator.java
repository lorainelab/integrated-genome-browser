...
public class QuickloadSourceValidator {
    ...
    public static final String[] URI_ERROR = { "Invalid URL.  This field should be of the form http://domain.com/path/to/rsc"
                , "The URL does not point to a chromeinfo.txt files"
                , "Unable to load locate the resource"
                , "The resource format is not valid.  Entries should be <sequence name> <tab> <sequence count>"
                , "Empty value not allowed"
    };
    
    public static boolean validateModchromeURI( String url_string, ErrorListener listener ){
        boolean valid = false;
        String UriErrorString = "";
        URI UriValue;
        if( url_string.equals("")){
            valid = false;
            UriErrorString = URI_ERROR[4];
        }else{
            try{ 
                UriValue = URI.create(url_string);
                
                if ( !UriValue.toString().endsWith( QuickloadSourceCreator.MOD_CHROM_INFO_FILE_NAME )){
                    valid = false;
                    UriErrorString=URI_ERROR[1];
                }else{
                    URL modchrome_path;
                    try {
                        modchrome_path = UriValue.toURL();
                        URLConnection modchrome_c = null;
                        BufferedReader modchrome_reader = null;
                        ...
                        valid = false;
                        UriErrorString=URI_ERROR[0];
                        ex.printStackTrace();
                    } catch (IOException ex) {
                        ex.printStackTrace();    
                        UriErrorString=URI_ERROR[2];
                    }
        ...
        if( listener != null){
            listener.setError( UriErrorString );
        }
        return valid;
    }
