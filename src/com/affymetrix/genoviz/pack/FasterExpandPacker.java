...
import com.affymetrix.genoviz.util.DoubleList;
import com.affymetrix.genoviz.tiers.*;
import com.affymetrix.genoviz.pack.EfficientExpandPacker;
 ...
 */
	...
	if (slot_max < child_min) {
	  // move child to slot;
	  double new_ycoord;
	  if (this.getMoveType() == Direction.UP) {  // stacking up for layout
	...
	if ((max_slots_allowed > 0) && slot_maxes.size() >= max_slots_allowed) {
          child.setVisibility(true);
	  if (this.getMoveType() == Direction.UP) {
