...
public class TierGlyph extends SolidGlyph {
  ...
  protected CharSequence label = null;
 ...
 */
  ...
  // overriding addChild() to keep track of whether children are sorted in child list
  //    by ascending min
  @Override
...
//        this.setCoords(mbox.x, cbox.y - p.y, mbox.width, cbox.height + p.y);
        ...
        this.setCoords(cbox.x, cbox.y - 6, cbox.width, cbox.height + 6);
      } else {
  ...
  public void draw(ViewI view) {
    ...
    if (middle_glyphs.size() == 0) { // no middle glyphs, so use fill color to fill entire tier
      if (iastyle.getBackground() != null) {
	g.setColor(iastyle.getBackground());
    ...
    for (int i=0; i<mcount; i++) {
      ...
      mglyph.setColor(iastyle.getBackground());
...
//          (int) rect2d.getWidth(), (int) rect2d.getHeight());
      ...
      g.drawString(getLabel().toString(), (hpix.x + hpix.width + 1), (hpix.y + fm.getMaxAscent() - 1));
  ...
  public void drawHandle(ViewI view) {
    ...
    if (hpix != null) {
      Graphics g = view.getGraphics();
      Color c = new Color(iastyle.getColor().getRed(), iastyle.getColor().getGreen(), iastyle.getColor().getBlue(), 64);
      g.setColor(c);
  ...
  public void setFillColor(Color col) {
    if (iastyle.getBackground() != col) {
      iastyle.setBackground(col);
    }
  ...
  public Color getFillColor() {
    return iastyle.getBackground();
  }

  @Override
  public void setForegroundColor(Color color) {
    if (iastyle.getColor() != color) {
      iastyle.setColor(color);
    }
